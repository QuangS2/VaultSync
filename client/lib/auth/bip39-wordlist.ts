/**
 * Standard BIP-39 English Wordlist & Mnemonic Generation / Validation
 * Generates 12-word cryptographic seed phrases with 128-bit entropy + 4-bit SHA-256 checksum.
 */
...
export const BIP39_ENGLISH_WORDLIST: readonly string[] = [
  ...
  'secret', 'section', 'security', 'seduce', 'see', 'seed', 'seek', 'segment', 'select', 'sell',
  ...
  'stable', 'stadium', 'staff', 'stage', 'stain', 'stairs', 'stamp', 'stand', 'start', 'state',
  ...
  'vacant', 'vacuum', 'vague', 'valuable', 'van', 'vanish', 'vapor', 'various', 'vast', 'vault',
...
/**
 * Generates a 12-word BIP-39 mnemonic phrase from 128-bit cryptographically secure entropy.
 */
export async function generateMnemonic12(): Promise<string> {
  const entropy = new Uint8Array(16); // 128 bits
  crypto.getRandomValues(entropy);

  // Compute SHA-256 hash to extract 4-bit checksum
  const hashBuffer = await crypto.subtle.digest('SHA-256', entropy);
  const hashBytes = new Uint8Array(hashBuffer);
  const firstByte = hashBytes[0] ?? 0;
  const checksumByte = firstByte >> 4; // Top 4 bits

  // Convert 128 bits + 4 checksum bits = 132 bits into 12 groups of 11-bit indices
  const bits: number[] = [];
  for (let i = 0; i < entropy.length; i++) {
    const eByte = entropy[i] ?? 0;
    for (let b = 7; b >= 0; b--) {
      bits.push((eByte >> b) & 1);
    }
  }
  ...
  for (let i = 0; i < 12; i++) {
    let index = 0;
    for (let b = 0; b < 11; b++) {
      const bit = bits[i * 11 + b] ?? 0;
      index = (index << 1) | bit;
    }
    const word = BIP39_ENGLISH_WORDLIST[index] ?? 'abandon';
    words.push(word);
  }
...
/**
 * Validates a 12-word mnemonic phrase against the BIP-39 dictionary and checksum.
 */
export async function validateMnemonic12(phrase: string): Promise<boolean> {
  ...
  // Extract 4 bits checksum
  let checksumExpected = 0;
  for (let b = 0; b < 4; b++) {
    const bit = bits[128 + b] ?? 0;
    checksumExpected = (checksumExpected << 1) | bit;
  }
  ...
  const hashBytes = new Uint8Array(hashBuffer);
  const firstByte = hashBytes[0] ?? 0;
  const actualChecksum = firstByte >> 4;
